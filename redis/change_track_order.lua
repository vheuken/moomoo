...
function get_track_num_from_id (id, track_order)
  return tonumber(track_order[index_of(id)-1])
end
...
if track_num > destination_track_num then
  ...
  for i=1, #raw_track_order-1, 2 do
    ...
    if n >= track_num and n <= destination_track_num then
      raw_track_order[i] = tostring(n+10)
    end
  end
else
...
raw_track_order[index_of(track_id)-1] = tostring(destination_track_num)
